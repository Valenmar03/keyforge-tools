...
export default function ToolLayout({ tool, children, locale }: Props) {
  ...
  function absoluteUrl(path: string) {
    // En prod: NEXT_PUBLIC_SITE_URL=https://tudominio.com
    const base = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
    return `${base}${withLocale(path)}`;
  }
  ...
  const faqSchema =
    toolText.faqItems?.length > 0
      ? {
          "@context": "https://schema.org",
          "@type": "FAQPage",
          mainEntity: toolText.faqItems.map((item) => ({
            "@type": "Question",
            name: item.question,
            acceptedAnswer: {
              "@type": "Answer",
  ...
  // JSON-LD Breadcrumb Schema (localizado)
  ...
  // Asegura que links internos en markdown apunten a /{locale}/...
  const processedSeoContent = toolText.seoContent?.replace(
    /\[([^\]]+)\]\(\/tools\/([^)]+)\)/g,
      ...
      <section className="relative bg-gradient-to-br from-slate-50 via-white to-blue-50 pt-8 pb-12 border-b border-slate-100">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <nav className="flex flex-wrap items-center gap-2 text-sm text-slate-500 mb-6">
            ...
            <Link href={withLocale("/tools")} className="hover:text-slate-900">
              {t("breadcrumbsTools")}
            </Link>
            <ChevronRight className="w-4 h-4" />
            <Link
              href={withLocale(`/tools?category=${tool.category}`)}
              className="hover:text-slate-900"
            >
              {categoryText?.name ?? tool.category}
            </Link>
            <ChevronRight className="w-4 h-4" />
            <span className="text-slate-900 font-medium">{toolText.title}</span>
          </nav>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-lg text-slate-600">{toolText.description}</p>

              {!!toolText.keywords?.length && (
                <div className="flex flex-wrap gap-2 mt-4">
                  {toolText.keywords.map((keyword) => (
                    <span
                      key={keyword}
        ...
        <section className="py-12 border-t border-slate-100">
          <div className="max-w-4xl mx-auto px-6">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              ...
              >
                {toolText.faqItems.map((item, idx) => (
                  <AccordionItem
                    key={idx}
                    value={`faq-${idx}`}
              ...
              {relatedTools.map((relatedTool) => {
                ...
                return (
                  <Link
                    ...
                    href={withLocale(`/tools/${relatedTool.slug}`)}
      ...
      <section className="py-8 bg-white border-t border-slate-100">
        <div className="max-w-4xl mx-auto px-6">
          <div className="flex items-center gap-3 text-sm text-slate-500">
            ...
            <span>
              <strong className="text-slate-700">{t("privacyNoteTitle")}</strong>{" "}
              {t("privacyNoteBody")}
            </span>
          </div>
        </div>
